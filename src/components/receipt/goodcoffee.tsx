...
export function buildGoodcoffeeReceiptHtml(data: ReceiptTemplateData) {
	return `
	<html>
	<head>
		<title>${escapeHtml(data.shopName)} Receipt - ${escapeHtml(data.referenceNumber)}</title>
		<style>
			body { font-family: 'Courier New', monospace; padding: 20px; font-size: 12px; max-width: 300px; margin: 0 auto; color: #422006; }
			.brand { text-align: center; color: #a16207; font-weight: 800; font-size: 18px; margin-bottom: 2px; }
			.ref { text-align: center; font-size: 11px; color: #713f12; }
			.divider { border-top: 1px dashed #713f12; margin: 10px 0; }
			.row { display: flex; justify-content: space-between; margin: 5px 0; gap: 8px; }
			.row span:last-child { text-align: right; }
			.bold { font-weight: 700; }
			.total { font-size: 14px; color: #ca8a04; font-weight: 800; }
			.footer { text-align: center; margin-top: 14px; font-size: 10px; color: #713f12; }
		</style>
	</head>
	<body>
		...
		<div class="divider"></div>
		<div class="footer">${data.receiptFooter ? escapeHtml(data.receiptFooter) : "Thank you for visiting Good Coffee"}</div>
	</body>
	</html>
	`;
}
