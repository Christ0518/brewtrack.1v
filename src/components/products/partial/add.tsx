...
export default function AddProduct({ shopId = "1", onSuccess, onCancel }: AddProductProps) {
    ...
    const loadData = async () => {
      try {
        ...
        if (ingredientsResponse.success) {
          ...
          console.error("Failed to load ingredients:", ingredientsResponse.message);
          setIngredients([]);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await Fetch_to(api_links.tbl_products, formWithoutImage, {}, 3, 1000, "POST");
      ...
      if (imageBase64 && response.data?.id) {
        localStorage.setItem(`product_image_${response.data.id}`, imageBase64);
      }
        ...
        <div className="bg-white rounded-lg shadow-2xl max-w-md w-full animate-slideUp">
          ...
          <div className="flex items-center justify-between p-4 border-b border-slate-200">
            ...
            <button
              onClick={() => {
                setShowAddIngredientModal(false);
                setNewIngredient({ ingredient_name: "", quantity: "", unit: "kg" });
              }}
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <p className="text-xs text-slate-700">
                <span className="font-semibold text-[#073dbe]">Tip:</span> Add ingredients to your inventory for use in product variants.
              </p>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white border-b border-slate-200 sticky top-0 z-10 p-4 lg:p-6 flex items-center justify-between">
          <div>
            <h1 className="text-xl lg:text-2xl font-bold text-slate-900">Add New Product</h1>
            <p className="text-slate-600 text-sm mt-1">Create a new menu item with variants and ingredients</p>
          </div>
          <button
                ...
                {form.variants && form.variants.length > 0 ? (
                  form.variants.map((v, idx) => (
                    <div key={v.id || idx} className="bg-white rounded-lg p-3 border border-slate-200">
                      <div className={`grid gap-2 mb-2 ${form.variants.length > 1 ? "grid-cols-3" : "grid-cols-2"}`}>
                        <input
                          type="text"
